...
def upload_image(request):
    ...
    if request.method == 'POST':
        ...
        cmd = f"python {root}main.py eval --content-image {path} --style-image {style} --output-image {output} --model {model} --content-size {content_size}" 
        os.system(cmd)
        
        # 结果文件名字,带后缀
        # result = '0.png'

        return HttpResponse(json.dumps({'status': 0, 'message': '上传成功', 'result': output}))
